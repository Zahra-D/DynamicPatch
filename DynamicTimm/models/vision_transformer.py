...
from DynamicTimm.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD, IMAGENET_INCEPTION_MEAN, IMAGENET_INCEPTION_STD, \
    OPENAI_CLIP_MEAN, OPENAI_CLIP_STD
from DynamicTimm.layers import DynamicPatchEmbed, PatchEmbed, Mlp, DropPath, AttentionPoolLatent, RmsNorm, PatchDropout, SwiGLUPacked, \
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            embed_layer: Callable= DynamicPatchEmbed,
        ...
        self.dynamic_patch_embed = embed_layer(
            embed_dim = embed_dim,
            
            num_patches= 70
        )
        
        # self.dynamic_patch_embed = embed_layer_2()
        num_patches = self.dynamic_patch_embed.num_patches
        ...
        self.pos_embed = nn.Parameter(torch.randn(1386, embed_dim) * .02)
    ...
    def group_matcher(self, coarse: bool = False) -> Dict:
        return dict(
            stem=r'^cls_token|pos_embed|dynamic_patch_embed',  # stem and embed
    ...
    def _pos_embed(self, x: torch.Tensor, flatten_index, S) -> torch.Tensor:
        ...
        if self.no_embed_class:
            ...
            B = x.shape[0] 
            pos_embed = self.pos_embed[flatten_index]
            x = pos_embed + x* S.unsqueeze(2)
            # print("dfdfdfdfdf")
            if to_cat:
                x = torch.cat(to_cat + [x], dim=1)
...
#need modificaiotn to be consistent with dynamic_patch_embed
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        x, selected_patches_index, S = self.dynamic_patch_embed(x)
        x = self._pos_embed(x, selected_patches_index, S)
        x = self.patch_drop(x)
        x = self.norm_pre(x)
